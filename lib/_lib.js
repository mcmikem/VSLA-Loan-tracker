...
// ---------- CORS + methods ----------
// Same-origin app needs no CORS headers at all. Cross-origin callers only
// get a pass when they match the allowlist (production URL via APP_URL,
// Vercel previews, local dev). Everything else is browser-blocked.
...
export function cors(req, res, methods = 'GET,POST,OPTIONS') {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, x-group-id, x-admin-key, Authorization');
