...
/** Existence check WITHOUT creating anything (unlike loadGroup). */
export async function groupExists(groupId) {
  if (!usePostgres()) return memory.has(groupId);
  try {
    await ensureTable();
    const p = await getPool();
    const { rows } = await p.query('SELECT 1 FROM vsla_app_state WHERE group_id = $1 LIMIT 1', [groupId]);
    return rows.length > 0;
  } catch {
    return true; // fail closed — treat as existing so writes stay gated
  }
}
