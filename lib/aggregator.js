...
export async function initiateUgCharge(args) {
  ...
  if (!flwId) throw new AggregatorError('Flutterwave gave no transaction id. No money was taken.', 502);
  return {
    flwId,
...
export async function verifyFlwTransaction(flwId) {
  if (!flwId) throw new AggregatorError('Missing Flutterwave transaction id.', 400);
  const { response, payload } = await fetchJson(
    `${FLW_BASE}/transactions/${encodeURIComponent(String(flwId))}/verify`,
