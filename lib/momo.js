...
export async function requestCollection(input, ctx = {}) {
  ...
  if (config.aggregatorEnabled) {
    ...
    return { transactionId: txRef, provider: input.network, via: 'flutterwave', status: 'pending', mode: 'live', redirectUrl: started.redirectUrl };
  }
...
export async function collectionStatus(network, transactionId) {
  ...
  if (pending && pending.provider === 'flutterwave') {
    ...
    const verified = await verifyFlwTransaction(pending.flwId, pending.txRef);
    const status = verified.status;
    if (status === 'confirmed' || status === 'failed') {
      await savePendingTx({ ...pending, status, verifiedAmount: verified.amount, verifiedAt: new Date().toISOString() });
    }
    return { transactionId: pending.txRef, provider: pending.network || network, via: 'flutterwave', status, mode: 'live', amount: verified.amount, redirectUrl: pending.redirectUrl || null };
  }
