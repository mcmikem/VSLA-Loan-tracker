    ...
    <main className="w-full max-w-lg mx-auto px-4 pt-4 pb-14 flex-1 space-y-4">
      ...
      {showAdd && (
        <section className="bg-white rounded-xl border border-[#E5E7EB] p-4 space-y-2">
          ...
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={str('Product name (e.g. Maize flour)', 'Erinya ly\'ekintu')} className="w-full min-h-[44px] border border-[#E5E7EB] rounded-lg px-3 text-sm" />
          {tab === 'member' && (
        ...
        <div key={p.id} className="bg-white rounded-xl border border-[#E5E7EB] p-3.5 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-[#4B5563]">
              ...
              {str('Cost', 'Omutindo')} <span className="font-mono">{p.costPrice.toLocaleString()}</span> · {str('Price', 'Omutengo')} <span className="font-mono font-bold">{p.salePrice.toLocaleString()}</span>/{p.unit}
            </p>
            <p className="text-[11px] font-mono text-[#4B5563]">
              {str('Stock:', 'Zisigadde:')} {p.stockQty} · {str('Sold:', 'Zitundiddwa:')} {p.soldQty}
            </p>
          </div>
          <button
            type="button"
