    ...
    <main className="w-full max-w-lg mx-auto px-4 pt-4 pb-14 flex-1 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-[#4B5563]">{str('Stock, sales & profit back to the loan fund', 'Magoba gadde mu loan')}</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="flex bg-white p-1 rounded-xl border border-[#CBD5E1] text-xs font-bold">
        <button type="button" onClick={() => setTab('group')} className={`flex-1 py-2 rounded-lg transition ${tab === 'group' ? 'bg-[#0b3d2e] text-white' : 'text-[#4B5563]'}`}>
          {str('Group stock', 'Ebintu by\'ekibiina')}
        </button>
        <button type="button" onClick={() => setTab('member')} className={`flex-1 py-2 rounded-lg transition ${tab === 'member' ? 'bg-[#0b3d2e] text-white' : 'text-[#4B5563]'}`}>
          {str('Member businesses', 'Bizinesi za members')}
        </button>
      </div>
      ...
      {showAdd && (
        <section className="bg-white rounded-xl border border-[#E5E7EB] p-4 space-y-2">
          <h3 className="text-xs font-bold text-[#00261b] uppercase tracking-wider">
            {tab === 'group' ? str('Buy stock for the group', 'Gula ebintu by\'ekibiina') : str('List a member business', 'Yongerako business')}
          </h3>
          {tab === 'group' && (
            <p className="text-[11px] text-[#4B5563]">
              {str('Cash in box:', 'Ensimbi mu Akasanduuko:')} <span className="font-mono font-bold">UGX {boxCashBalance.toLocaleString()}</span>
            </p>
          )}
          ...
          {tab === 'member' && (
            <>
              ...
              <input value={form.sellerName} onChange={(e) => setForm({ ...form, sellerName: e.target.value })} placeholder={str('Seller name', 'Erinnya lya member')} className="w-full min-h-[44px] border border-[#E5E7EB] rounded-lg px-3 text-sm" />
              <input value={form.sellerPhone} onChange={(e) => setForm({ ...form, sellerPhone: e.target.value })} inputMode="tel" placeholder={str('Seller phone (buyers contact you here)', 'Essimu ya member')} className="w-full min-h-[44px] border border-[#E5E7EB] rounded-lg px-3 text-sm font-mono" />
            </>
          )}
          <div className="grid grid-cols-3 gap-2">
            <input value={form.costPrice} onChange={(e) => setForm({ ...form, costPrice: e.target.value })} inputMode="numeric" placeholder={str('Cost', 'Cost')} className="min-h-[44px] border border-[#E5E7EB] rounded-lg px-2 text-sm font-mono" />
            <input value={form.salePrice} onChange={(e) => setForm({ ...form, salePrice: e.target.value })} inputMode="numeric" placeholder={str('Price', 'Omuwendo')} className="min-h-[44px] border border-[#E5E7EB] rounded-lg px-2 text-sm font-mono" />
            <input value={form.stockQty} onChange={(e) => setForm({ ...form, stockQty: e.target.value })} inputMode="numeric" placeholder={str('Qty', 'Obungi')} className="min-h-[44px] border border-[#E5E7EB] rounded-lg px-2 text-sm font-mono" />
          </div>
          ...
          <button type="button" onClick={submitAdd} className="w-full min-h-[48px] bg-[#006d30] text-white rounded-lg font-bold text-sm active:scale-[0.99]">
            {tab === 'group' ? str('Buy stock from box cash', 'Gula') : str('List business', 'Wandiisa')}
          </button>
        </section>
      )}
      ...
      {visible.length === 0 && (
        <div className="bg-[#FFF8E1] border-2 border-[#EAB308] rounded-xl p-4 text-center space-y-2">
          <p className="text-xs font-bold text-[#00261b]">
            {tab === 'member'
              ? str('No ventures yet — be the first! List your produce, tailoring, bodaboda, salon…', 'Tewali business — ba asooka! Yongerako ebintu byo oba omulembe…')
        ...
        <div key={p.id} className="bg-white rounded-xl border border-[#E5E7EB] p-3.5 space-y-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-[#4B5563]">
              ...
              {str('Price', 'Omuwendo')} <span className="font-mono font-bold">{p.salePrice.toLocaleString()}</span>/{p.unit}
            </p>
      ...
      {sellProduct && (
        <section className="bg-white rounded-xl border-2 border-[#00261b] p-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input value={sellForm.qty} onChange={(e) => setSellForm({ ...sellForm, qty: e.target.value })} inputMode="numeric" placeholder={str('Qty', 'Obungi')} className="min-h-[44px] border border-[#E5E7EB] rounded-lg px-3 text-sm font-mono" />
            <input value={sellForm.unitPrice} onChange={(e) => setSellForm({ ...sellForm, unitPrice: e.target.value })} inputMode="numeric" placeholder={str('Unit price', 'Omuwendo')} className="min-h-[44px] border border-[#E5E7EB] rounded-lg px-3 text-sm font-mono" />
          </div>
      ...
      {tab === 'group' && (
        <section className="bg-white rounded-xl border border-[#E5E7EB] p-4 space-y-2">
          <h3 className="text-xs font-bold text-[#00261b] uppercase tracking-wider">{str('Shop expense', 'Ensimbi ezisaasaanye')}</h3>
          <div className="flex gap-2">
