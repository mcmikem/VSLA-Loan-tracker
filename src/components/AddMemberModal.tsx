    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-end sm:items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-5 space-y-2.5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-[#00261b]">{str('Register new member', 'Wandiisa omukiise', 'Sajili mwanachama')}</h2>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input value={form.firstName} onChange={(e) => set('firstName', e.target.value)} placeholder={str('First name', 'Erinya', 'Jina')} className={inputCls} />
          <input value={form.lastName} onChange={(e) => set('lastName', e.target.value)} placeholder={str('Last name', 'Erinya ly\'ekika', 'Jina la ukoo')} className={inputCls} />
        </div>
        <div className="grid grid-cols-2 gap-2">
        ...
        <input value={form.nationalId} onChange={(e) => set('nationalId', e.target.value)} placeholder={str('National ID (optional)', 'Endaga muntu', 'Kitambulisho')} className={inputCls} />
        <div className="grid grid-cols-2 gap-2">
          <input value={form.village} onChange={(e) => set('village', e.target.value)} placeholder={str('Village / zone', 'Ekyalo', 'Kijiji')} className={inputCls} />
          <input value={form.business} onChange={(e) => set('business', e.target.value)} placeholder={str('Business', 'Bizinesi', 'Biashara')} className={inputCls} />
        </div>
        <p className="text-[11px] font-bold text-[#4B5563] uppercase">{str('Next of kin', 'Ow\'oluganda', 'Ndugu wa karibu')}</p>
        <div className="grid grid-cols-2 gap-2">
          <input value={form.kinName} onChange={(e) => set('kinName', e.target.value)} placeholder={str('Kin name', 'Erinya', 'Jina')} className={inputCls} />
        ...
        <p className="text-[11px] font-bold text-[#4B5563] uppercase">{str('Guarantor (member)', 'Omuyima', 'Mdhamini')}</p>
        <div className="grid grid-cols-2 gap-2">
          <input value={form.guarantorName} onChange={(e) => set('guarantorName', e.target.value)} placeholder={str('Guarantor name', 'Erinya ly\'omuyima', 'Jina la mdhamini')} className={inputCls} />
        ...
        <button type="button" onClick={submit} className="w-full min-h-[48px] bg-[#006d30] text-white rounded-lg font-bold text-sm active:scale-[0.99]">
          {str('Register member', 'Wandiisa', 'Sajili')}
        </button>
